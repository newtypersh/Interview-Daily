import { Interview, InterviewAnswer, Question, QuestionSet } from "@prisma/client";

type InterviewAnswerWithRelations = InterviewAnswer & {
    question?: Question | null;
};

type InterviewWithRelations = Interview & {
    questionSet?: Partial<QuestionSet> | null;
    answers?: InterviewAnswerWithRelations[];
};

export function toInterviewAnswerDto(a: InterviewAnswerWithRelations | null) {
    if (!a) return null;
    return {
        id: a.id != null ? String(a.id) : null,
        interviewId: a.interview_id != null ? String(a.interview_id) : null,
...
export function toInterviewDto(i: InterviewWithRelations | null) {
    if (!i) return null;
    return {
