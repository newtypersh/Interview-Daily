...
export const QuestionSetNameSchema = z.string()
...
// 개별 질문 내용
export const QuestionContentStringSchema = z.string()
  .min(2, '질문은 최소 2자 이상이어야 합니다.')
  .max(200, '질문은 200자 이내로 작성해주세요.');

export const QuestionContentSchema = z.object({
  content: QuestionContentStringSchema,
});

// 피드백 템플릿 마크다운 내용
export const FeedbackTemplateContentStringSchema = z.string()
    .min(1, '템플릿 내용을 입력해주세요.')
    .max(3000, '템플릿 내용은 3000자 이내로 작성해주세요.');

export const FeedbackTemplateContentSchema = z.object({
  content: FeedbackTemplateContentStringSchema,
});
